...
export default function Create({ auth }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        gallery: [],
        addons: [],
        includes: { included: [], excluded: [] }, // ✅ Initialize Includes
        translations: {
            en: { name: "", description: "", location: "", category: "", itinerary: "", notes: "" }, // ✅ Text Fields
            id: { name: "", description: "", location: "", category: "", itinerary: "", notes: "" },
        },
    });
    ...
    const handleTranslationChange = (locale, key, value) => {
        setData("translations", {
            ...data.translations,
            [locale]: { ...data.translations[locale], [key]: value },
        });
    };
                ...
                <form onSubmit={submit} className="space-y-6">
                    ...
                    <Tabs defaultValue="core" className="w-full">
                        <TabsList className="grid w-full grid-cols-4">
                            ...
                            <TabsTrigger value="includes">Included/Excluded</TabsTrigger> {/* ✅ New Tab */}
                        ...
                        <TabsContent value="includes" className="mt-4">
                            <Card>
                                <CardHeader>
                                    <CardTitle>Included & Excluded</CardTitle>
                                    <CardDescription>Manage lists of what is included and excluded in the price.</CardDescription>
                                </CardHeader>
                                <CardContent>
                                    <IncludesRepeater
                                        data={data.includes}
...
function TranslationForm({ locale, data, errors, onChange }) {
    return (
        <div className="space-y-4 animate-in fade-in slide-in-from-left-4 duration-300">
            <div>
                <Label htmlFor={`name_${locale}`}>Activity Name ({locale.toUpperCase()}) <span className="text-red-500">*</span></Label>
                <Input id={`name_${locale}`} value={data.name} onChange={(e) => onChange(locale, "name", e.target.value)} />
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <Input id={`location_${locale}`} value={data.location} onChange={(e) => onChange(locale, "location", e.target.value)} />
                    <InputError message={errors[`translations.${locale}.location`]} className="mt-1" />
                </div>
                <div>
                    <Label htmlFor={`category_${locale}`}>Category</Label>
                    <Input id={`category_${locale}`} value={data.category} onChange={(e) => onChange(locale, "category", e.target.value)} placeholder="e.g. Water Sport" />
                    <InputError message={errors[`translations.${locale}.category`]} className="mt-1" />
                </div>
            </div>

            <div>
                <Label htmlFor={`desc_${locale}`}>Description</Label>
                <Textarea id={`desc_${locale}`} value={data.description} onChange={(e) => onChange(locale, "description", e.target.value)} rows={4} />
                <InputError message={errors[`translations.${locale}.description`]} className="mt-1" />
            </div>

            {/* ✅ Translatable Text Fields */}
            <div>
                <Label htmlFor={`itinerary_${locale}`}>Itinerary</Label>
                <Textarea
                    id={`itinerary_${locale}`}
            ...
            <div>
                <Label htmlFor={`notes_${locale}`}>Important Notes</Label>
                <Textarea
                    id={`notes_${locale}`}
                    value={data.notes}
